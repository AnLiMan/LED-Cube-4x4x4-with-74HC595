...
void loop() {
  ...
  EffectSpeed = random(50, 120) + DeltaEffectSpeed; // изменение скорости анимации
...
void EffectB()
{
  FOR_i(0, 4) {
    ...
    for (byte j = 0; j < 16;) {
      for (byte k = i; k < 16;) {
        regWrite(k, HIGH);
        if (millis() - myTimer1 >= 0.25 * EffectSpeed) {
          myTimer1 += EffectSpeed;
          regWrite(k, LOW);
          k++;
        }
